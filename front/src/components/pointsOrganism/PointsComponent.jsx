    ...
    <div style={{ display: 'flex', justifyContent: 'center', margin: '5px' }}>
      {chakrasData.map((data, index) => (
        <div key={index} onClick={(event) => handleClick(index,event)} style={{ textAlign: 'center' }}>
          {(selectedChakra === null || selectedChakra === index) && <img src={data.imageSrc} alt="Chakra images" style={{marginLeft:14, width: 100, height:100}} />}
          {(selectedChakra === null || selectedChakra === index) && showText &&
            <Card variant="outlined" sx={{ backgroundColor: data.backgroundColor , borderRadius:6 , marginTop:10, }}>
              <CardContent>
